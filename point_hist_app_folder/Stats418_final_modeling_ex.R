...
## MODEL SUMMARY AND EXAMPLE SECTION

# CATEGORY 1 - FOR POINTS: lm(PTS ~ MP + Ptavg + DRtg + `eFG%`, data=df_train); median RMSE of 7.482


## THIS IS AN EXAMPLE OF HOW TO GET A POINT PREDICTION FOR A PLAYER / TEAM COMBO
## JUST REPLACE "PASCAL SIAKAM" WITH A PLAYER NAME, AND "ATLANTA HAWKS" WITH A TEAM NAME


siakpts <- game_logs23[["Pascal Siakam"]][nrow(game_logs23[["Pascal Siakam"]]),] %>% select(MP,Ptavg)

tmstd_pts <- oppjoin23 %>% select(Team,DRtg,`eFG%`) %>% mutate(DRtg = as.numeric(DRtg),`eFG%` = as.numeric(`eFG%`))

tmstd_pts2 <- tmstd_pts %>% mutate_at(c(names(tmstd_pts)[c(2,3)]), ~(scale(.) %>% as.vector))
  
atltmp <- tmstd_pts2 %>% filter(Team == "Atlanta Hawks")

siakptsj <- cbind(siakpts,atltmp)

mod1p <- lm(PTS ~ MP + Ptavg + DRtg + `eFG%`, data=game_logs23[["Pascal Siakam"]])

predict(mod1p,siakptsj)
...
# CATEGORY 2 - FOR REBOUNDS: lm(TRB.x ~ `eFG%` + `oeFG%` + BLK.y + `oFT/FGA`,data=df_train); median RMSE of 3.105

## THIS IS AN EXAMPLE OF HOW TO GET A REBOUND PREDICTION FOR A PLAYER / TEAM COMBO
## JUST REPLACE "PASCAL SIAKAM" WITH A PLAYER NAME, AND "ATLANTA HAWKS" WITH A TEAM NAME

tmstd_rebs <- oppjoin23 %>% select(Team,BLK,`eFG%`,`oeFG%`,`oFT/FGA`) %>% 
  summarize(Team = Team,BLK.y = as.numeric(BLK),`eFG%` = as.numeric(`eFG%`),`oeFG%` = as.numeric(`oeFG%`),`oFT/FGA` = as.numeric(`oFT/FGA`))

tmstd_rebs2 <- tmstd_rebs %>% mutate_at(c(names(tmstd_rebs)[c(2,3,4,5)]), ~(scale(.) %>% as.vector))

atltmr <- tmstd_rebs2 %>% filter(Team == "Atlanta Hawks")

mod1r <- lm(TRB.x ~ `eFG%` + `oeFG%` + BLK.y + `oFT/FGA`, data=game_logs23[["Pascal Siakam"]])

predict(mod1r,atltmr)

#########################################################################################
#########################################################################################
#########################################################################################

# CATEGORY 3 - FOR ASSISTS: lm(AST.x ~ `eFG%` + GmSc + FG + `+/-` + Pace,data=df_train); median RMSE of 2.099

## THIS IS AN EXAMPLE OF HOW TO GET AN ASSIST PREDICTION FOR A PLAYER / TEAM COMBO
## JUST REPLACE "PASCAL SIAKAM" WITH A PLAYER NAME, AND "ATLANTA HAWKS" WITH A TEAM NAME


siakast <- game_logs23[["Pascal Siakam"]][nrow(game_logs23[["Pascal Siakam"]]),] %>% select(GmSc,FG,`+/-`)

tmstd_ast <- oppjoin23 %>% select(Team,`eFG%`,Pace) %>% mutate(`eFG%` = as.numeric(`eFG%`),Pace = as.numeric(Pace))

tmstd_ast2 <- tmstd_ast %>% mutate_at(c(names(tmstd_ast)[c(2,3)]), ~(scale(.) %>% as.vector))

atltma <- tmstd_ast2 %>% filter(Team == "Atlanta Hawks")

siakastj <- cbind(siakast,atltma)

mod1a <- lm(AST.x ~ `eFG%` + GmSc + FG + `+/-` + Pace, data=game_logs23[["Pascal Siakam"]])

predict(mod1a,siakastj)
